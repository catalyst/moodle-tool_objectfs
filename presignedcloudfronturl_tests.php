...
/**
 * Pre-Signed CLOUDFRONT URL testing page
 *
...
admin_externalpage_setup('tool_objectfs_presignedcloudfronturl_testing');

$output = $PAGE->get_renderer('tool_objectfs');

echo $output->header();
echo $output->heading(get_string('presignedcloudfronturl_testing:page', 'tool_objectfs'));
...
$settingslink = \html_writer::link(new \moodle_url('/admin/tool/objectfs/index.php'),
    get_string('presignedcloudfronturl_testing:objectfssettings', 'tool_objectfs'));

if ($support) {

    if ($config->enablepresignedcloudfronturls) {
        ...
        if ($client and $client->get_availability()) {
            ...
            if ($connection->success) {
                ...
                if ($config->enablepresignedcloudfronturls) {
                    echo $output->presignedcloudfronturl_tests_content($fs, $testfiles);
                }
        ...
        echo $output->notification(get_string('presignedcloudfronturl_testing:presignedcloudfronturlsnotsupported', 'tool_objectfs'), 'notifyproblem');
        echo $output->heading(get_string('presignedcloudfronturl_testing:checkfssettings', 'tool_objectfs').$settingslink, 5);
    }
